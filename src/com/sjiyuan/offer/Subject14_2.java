 ...
 * 剪绳子2
 */
public class Subject14_2 {
    /**
     * 存放对应位数，整数拆分，作为其中一个数字时，最优解
     */
    private int[] max = new int[1000];
    ...
    public int cuttingRope(int n) {
        ...
        for (int i = 4; i <= n; i++) {
            ...
            int temp = 0;
            for (int j = 1; j <= i / 2; j++) {
                if ((max[j] * max[i - j]) % 1000000007 > temp) temp = (max[j] * max[i - j]) % 1000000007;
            }
            max[i] = temp % 1000000007;
        }
    ...
    public static void main(String[] args) {
        Subject14_2 subject14_2 = new Subject14_2();
    }
}
